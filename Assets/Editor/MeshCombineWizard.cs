...
public class MeshCombineWizard : ScriptableWizard {
    ...
    void OnWizardCreate() {
		...
		totalCount+=meshesWithSameMaterial[i].sharedMesh.vertexCount;
		if (totalCount>64000)
		{
		subsets.Add(i);
	        totalCount=meshesWithSameMaterial[i].sharedMesh.vertexCount;
	    ...
	    subsets.Add(meshesWithSameMaterial.Count);
	    for(int mn = 0; mn < subsets.Count-1; mn++)
	    {
            CombineInstance[] combine = new CombineInstance[meshesWithSameMaterial.Count];
	    for(int i = subsets[mn]; i <subsets[mn+1]; i++) {
	    ...
	    materialName = entry.Key.ToString().Split(' ')[0];
         
            combinedMesh.CombineMeshes(combine);
            materialName += "_" + combinedMesh.GetInstanceID()+"_"+mn.ToString();
